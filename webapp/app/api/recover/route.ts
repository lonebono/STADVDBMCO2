...
export async function POST() {
  ...
  if (logs.length === 0) {
    return NextResponse.json({ message: "No pending transactions to recover." });
  }
  ...
  for (const entry of logs) {
    ...
    try {
      ...
      const { tconst, titleType, primaryTitle, startYear, runtimeMinutes } = entry.data;
      
      await conn.execute(query, [tconst, titleType, primaryTitle, startYear, runtimeMinutes]);
      await conn.commit(); 
  ...
  return NextResponse.json({ 
    ...
    message: recoveredCount > 0 ? `Successfully recovered ${recoveredCount} transactions!` : "No transactions recovered."
  });
}