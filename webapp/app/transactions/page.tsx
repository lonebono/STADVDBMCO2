      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            <h2 className="text-gray-400 font-bold mb-4 uppercase text-sm tracking-wider">Step 3: Concurrency Config</h2>
            
            <label className="text-white text-sm block mb-1">Isolation Level</label>
            <select
            ...
            >
              <option value="READ UNCOMMITTED">Read Uncommitted (Allows Dirty Reads)</option>
              <option value="READ COMMITTED">Read Committed (Standard)</option>
              <option value="REPEATABLE READ">Repeatable Read</option>
              <option value="SERIALIZABLE">Serializable (Strict Locking)</option>
            </select>
            ...
            <label className="text-white text-sm block mb-1">Artificial Delay (ms)</label>
            <input
              type="number"
            ...
            <p className="text-xs text-gray-500 mt-1">
              Set to <strong>5000</strong> to test locking behavior during concurrency.
            </p>
          </div>

          {/* data payload */}
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            <h2 className="text-gray-400 font-bold mb-4 uppercase text-sm tracking-wider">Data Payload</h2>
            
            <div className="space-y-3">
              <div>
                <label className="text-xs text-gray-400">ID (tconst)</label>
                <input className="w-full p-2 bg-gray-900 text-white border border-gray-600 rounded" 
                    value={tconst} onChange={e => setTconst(e.target.value)} />
              </div>
              ...
              <div className="flex gap-2">
                <div className="w-1/2">
                   ...
                   <input type="number" className="w-full p-2 bg-gray-900 text-white border border-gray-600 rounded" 
                       value={year} onChange={e => setYear(Number(e.target.value))} />
                </div>
                <div className="w-1/2">
                   <label className="text-xs text-gray-400">Runtime (min)</label>
                   <input type="number" className="w-full p-2 bg-gray-900 text-white border border-gray-600 rounded" 
                       value={runtime} onChange={e => setRuntime(Number(e.target.value))} />
                </div>
              </div>
            </div>
          </div>

          <Button onClick={submit} className="w-full py-4 font-bold text-lg shadow-lg shadow-blue-900/50 transition-transform active:scale-95">
            EXECUTE TRANSACTION
          </Button>
        </div>
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="mb-2 border-b border-gray-800 pb-2 text-gray-500 font-bold">Transaction Console Output</div>
            {statusLogs.length === 0 && <span className="text-gray-700 animate-pulse">Waiting for commands...</span>}
            {statusLogs.map((log, i) => (
                <div key={i} className="mb-2 border-b border-gray-900/50 pb-1 break-words whitespace-pre-wrap">
                    {log}
                </div>
            ))}
            </div>
